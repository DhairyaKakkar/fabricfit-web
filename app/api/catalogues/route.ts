...
export async function GET() {
  ...
  const { data } = await supabase
    ...
    .select(`
      *,
      catalogue_items(
        id, position, tryon_id,
        tryons(id, result_url, garment_type, status, purged_at)
      )
    `)
...
export async function POST(request: NextRequest) {
  ...
  const { data, error } = await supabase
    .from('catalogues')
    .insert({ name, description, outlet_id: outletId })
    .select()
    .single();
...
export async function DELETE(request: NextRequest) {
  ...
  await supabase.from('catalogues').delete().eq('id', id).eq('outlet_id', outletId ?? '');
