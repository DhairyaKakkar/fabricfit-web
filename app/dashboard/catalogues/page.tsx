...
export default function CataloguesPage() {
  ...
  const activeItems = activeCatalogue?.catalogue_items
    ?.filter(i => i.tryons?.status === 'success' && !i.tryons?.purged_at)
    ...
    <div>
        ...
        <div className="bg-white rounded-2xl border border-zinc-200 p-6 mb-6">
          <form onSubmit={handleCreate} className="flex flex-col sm:flex-row gap-3">
            ...
            <button
              ...
              className="bg-zinc-900 text-white rounded-xl px-5 py-2.5 text-sm font-semibold hover:bg-zinc-800 disabled:opacity-50 transition-colors shrink-0"
            >
      ...
      {loading ? (
        <div className="space-y-3">
          {[...Array(3)].map((_, i) => <div key={i} className="h-16 bg-zinc-100 rounded-2xl animate-pulse" />)}
        </div>
      ) : catalogues.length === 0 ? (
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className={`text-xs shrink-0 ${selected === cat.id ? 'text-zinc-400' : 'text-zinc-400'}`}>
                      {count} item{count !== 1 ? 's' : ''}
                    </span>
                  </div>
                  {cat.description && (
                    <p className={`text-xs mt-0.5 truncate ${selected === cat.id ? 'text-zinc-400' : 'text-zinc-400'}`}>
                      {cat.description}
                    </p>
                  )}
                </button>
          ...
          {selected && activeCatalogue && (
            <div className="flex-1">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  onClick={() => handleDelete(activeCatalogue.id)}
