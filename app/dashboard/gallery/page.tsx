...
export default function GalleryPage() {
  ...
  async function load() {
    ...
    setTryons(Array.isArray(data) ? data : []);
    setLoading(false);
  }
  ...
  const label = (t: TryOn) =>
    t.fabrics?.name ?? t.outfits?.name ?? t.garment_type;
              ...
              <div key={t.id} className="group relative rounded-2xl overflow-hidden bg-zinc-100 aspect-[3/4]">
                ...
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all duration-200 flex flex-col justify-between p-3 opacity-0 group-hover:opacity-100">
                  <button
                    ...
                    className="self-end bg-red-500 text-white text-xs font-semibold px-2.5 py-1 rounded-lg hover:bg-red-600 disabled:opacity-50 transition-colors"
                  >
