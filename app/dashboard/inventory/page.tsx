    ...
    <div>
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 p-6 mb-8">
        ...
        <form onSubmit={handleUpload} className="flex flex-col gap-3">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            />
            <input
              placeholder="Colour tag (e.g. Navy Blue)"
            ...
            <select
              ...
              className="rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900 transition capitalize"
            >
              {FABRIC_TYPES.map(t => <option key={t} value={t} className="capitalize">{t}</option>)}
            </select>
          </div>
          <div className="flex flex-col sm:flex-row gap-3">
            <input
      ...
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {[...Array(6)].map((_, i) => <div key={i} className="aspect-square bg-zinc-100 rounded-2xl animate-pulse" />)}
        </div>
      ) : fabrics.length === 0 ? (
                    ...
                    {fabric.fabric_garments.map(g => (
                      <span
                        ...
                        className={`text-[10px] px-1.5 py-0.5 rounded-full border capitalize ${
                          g.status === 'ready' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' :
                          g.status === 'failed' ? 'bg-red-50 border-red-200 text-red-600' :
