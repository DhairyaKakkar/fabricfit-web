...
export default function OutfitsClient({ initialOutfits, outletId }: { initialOutfits: Outfit[]; outletId: string }) {
  ...
  async function handleUpload(e: React.FormEvent) {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage.from('outfit_images').upload(path, file, { upsert: false });
      if (uploadError) throw new Error(uploadError.message);

      const { data: { publicUrl } } = supabase.storage.from('outfit_images').getPublicUrl(path);

      const res = await fetch('/api/outfits', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: name.trim() || null, image_url: publicUrl }) });
  ...
  async function handleDelete(id: string) {
    await fetch('/api/outfits', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) });
    ...
    <div>
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 p-6 mb-8">
        ...
        <form onSubmit={handleUpload} className="flex flex-col sm:flex-row gap-3">
          <input placeholder="Outfit name (optional)" value={name} onChange={e => setName(e.target.value)} className="flex-1 rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900 transition" />
          ...
          {outfits.map(outfit => (
            <div key={outfit.id} className="bg-white rounded-2xl border border-zinc-200 overflow-hidden group">
              ...
              <div className="p-3 flex items-center justify-between">
                ...
                <button onClick={() => handleDelete(outfit.id)} className="text-xs text-zinc-300 hover:text-red-500 transition-colors shrink-0 ml-2">Remove</button>
              </div>
            </div>
          ))}
        </div>
